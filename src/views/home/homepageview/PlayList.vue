  ...
  <div class="play-list mtop-60">
    ...
    <div class="jingpin" v-if="hasHighInfo">
      ...
      <div class="info-wrapper">
        ...
        <div class="mtop-20 font-16" style="color: white;" >
          <span>{{ highInfo.name }}</span>
        </div>
      </div>
    </div>
    <!-- 标签区域 -->
    <div class="list-tag mtop-20">
      <!-- 全部标签弹出层容器 -->
      <div class="layer-wrapper">
        ...
        <div class="layer" v-show="showLayer" ref="layerRef">
          <div class="h-80 font-16">全部歌单</div>
          <div class="div-line"></div>
          <div class="tag-list" v-for="(item, index) in allCats" :key="index">
            <div class="tag-title">
              <template v-if="index === 0">
                <i class="iconfont icon-diqiu font-24"></i
              ...
              <template v-if="index === 2">
                <i class="iconfont icon-changjing font-24"></i
                ><span class="mleft-6">场景</span>
              </template>
              <template v-if="index === 3">
                <i class="iconfont icon-xiaolian font-24"></i
                ><span class="mleft-6">情感</span>
              </template>
              <template v-if="index === 4">
                <i class="iconfont icon-dianji font-24"></i>
      ...
      <ul class="tag-ul">
		...
		<!-- 歌单列表 -->
    <el-skeleton class="mtop-10" v-show="isLoading" :rows="9" animated />
    <ImgList
      v-show="!isLoading"
...
<script>
import { getAllCat, getHotCat, getPlayListByCat, getHighquality } from '@/api/api_playlist'
...
export default {
  ...
  methods: {
    ...
    handleCurrentChange(val) {
      console.log('当前是第'+this.pageInfo.currentPage+'页')
			console.log('要转到'+val+'页')
